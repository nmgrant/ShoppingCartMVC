...
namespace Ch24ShoppingCartMVC.Models {
	public class OrderModel {
		...
		public List<Product> GetAllProductsFromDataStore() {
			using (HalloweenEntities data = new HalloweenEntities()) {  //get all the products from the Collection Products order by name using HalloweenEntities
				return data.Products.OrderBy(e => e.Name).ToList();
			}
		}
		...
		public List<ProductViewModel> GetAllProducts() {
			...
			foreach (Product p in products) {  //Call the method ConvertToViewModel to convert p and pass the method ConvertToViewModel to the method add of the productmodels
		...
		public Product GetProductByIdFromDataStore(string id) {
			using (HalloweenEntities data = new HalloweenEntities()) {  //Get a product from Products of data where ProductID is matched with id parameter
